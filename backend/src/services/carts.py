...
class CartsService:
    ...
    async def create_cart(
        ...
        await uow.carts.add_one(id=user_id, products=dict())

    @staticmethod
    async def get_cart(
            uow: AbstractUOW, user_id: int
    ) -> CartInfoSchema:
        cart = await uow.carts.find_one(id=user_id)
        return CartInfoSchema(**cart.dict())

    @staticmethod
    async def add_product(
            uow: AbstractUOW, user_id: int, product_id: int, count: int
    ):
        key = str(product_id)
        products = (await uow.carts.find_one(id=user_id)).products
